...
export const createUser = async (req, res) => {
    ...
    try {
        ...
        if (gmail === 'Flash@gmail.com' || gmail === 'Berry@gmail.com') {
            const newUser = new User({ ...req.body, password: hashedPassword, admin: true });
           await newUser.save();
        }

        const newUser = new User({ ...req.body, password:hashedPassword, admin: false, otp, otpExpires, AltimatAdmin: false, profile: {
            country: "", Number: "", Street: "", Bio: ""
        } });
        await newUser.save();
        ...
        try {
            const mailObj = {
                ...
                body: `
                    <h1>Welcome to Aliexpress <strong>${username}</strong> 😍</h1>
